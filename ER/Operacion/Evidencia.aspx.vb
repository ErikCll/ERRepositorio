...
Public Class Evidencia1
    ...
    Public Sub MostrarGridEvidencia()
        ...
        Dim Query = "SELECT TOP 1 id_evidencia,id_requisito,estado FROM Op_Ev_Req_E3 WHERE Id_Instalacion=" + CType(Me.Master, Site2).IdInstalacion.ToString() + " AND Id_requisito=" + id.ToString() + " AND estado in (1,2,0,3) AND activado IS NULL ORDER BY id_evidencia DESC"
        ...
        Else
            ...
            frame.Src = "https://eregional.blob.core.windows.net/opedocs/" & Id_Evidencia.ToString() & ".pdf"
            'frame.Src = "https://er2020.blob.core.windows.net/opedocs/" & Id_Evidencia.ToString() & ".pdf"
    ...
    Protected Sub btnGuardar_Click(sender As Object, e As EventArgs) Handles btnGuardar.Click
        ...
        If chkRequisito.Checked = True Then
            Dim sqlQuery As String = "INSERT INTO Op_Ev_Req_E3(id_requisito,estado,Observaciones,id_usuario,activado,id_Instalacion,fecha_registro) VALUES(" + id.ToString() + ",3,'" + txtDesc.Value + "'," + IdUsuario.ToString() + ",NULL," + CType(Me.Master, Site2).IdInstalacion.ToString() + ",NULL)"
        ...
        Else
            ...
            If File1.HasFile Then
                ...
                If txtFecha.Text = "" Then
                    Dim txtJS As String = String.Format("<script>alert('{0}');</script>", "Seleccionar fecha de la evidencia.")
                    ScriptManager.RegisterClientScriptBlock(litControl, litControl.GetType(), "script", txtJS, False)
                Else


                    If fileExt = ".pdf" Or fileExt = ".PDF" Then
                        Dim sqlQuery As String = "INSERT INTO Op_Ev_Req_E3(id_requisito,estado,Observaciones,id_usuario,activado,id_Instalacion,fecha_requisito,fecha_registro) VALUES(" + id.ToString() + ",0,'" + txtDesc.Value + "'," + IdUsuario.ToString() + ",NULL," + CType(Me.Master, Site2).IdInstalacion.ToString() + ",CONVERT(date,'" + txtFecha.Text + "',103),DATEADD(HH, -5, GETDATE()))"
                        If obj.Insertar(sqlQuery) Then
                            MostrarGridEvidencia()
                            MostrarGridEvidencia2()
                            Dim Query = "SELECT TOP 1 id_evidencia FROM Op_Ev_Req_E3 WHERE Id_Instalacion=" + CType(Me.Master, Site2).IdInstalacion.ToString() + " AND Id_requisito=" + id.ToString() + " AND estado in (1,2,0) ORDER BY id_evidencia DESC"
                            ...
                            Dim sampleContainer As CloudBlobContainer = client.GetContainerReference("opedocs")
                            ...
                            Dim mensaje As String = "<!doctype html> <html>" &
                                "<body>Evidencia cargada para el requisito: <b>" + Requisito + "</b>, Instalación: <b>" + CType(Me.Master, Site2).Instalacion.ToString() + "</b>, Etapa: <b>Operación ES</b><hr>" &
                    "<br>Creado por el usuario: <b>" + Page.User.Identity.Name.ToString() + "</b>, el día <b>" + DateTime.Now.AddHours(-5).ToLongDateString() + ".</b><br>" &
                            ...
                            Dim queryCorreo As String = "SELECT Email FROM Usuario us JOIN Op_UsIns ins on us.Id_usuario=ins.Id_Usuario WHERE ins.Id_Instalacion=" + CType(Me.Master, Site2).IdInstalacion.ToString() + " AND us.EsOperador=1 AND us.Activado IS NULL"
    ...
    Public Sub MostrarGridEvidencia2()
        Dim decodedString As String = System.Text.ASCIIEncoding.ASCII.GetString(Convert.FromBase64String(Request.QueryString("id")))
        Dim id As String = decodedString
        Dim Query As String = "SELECT id_evidencia, CASE WHEN estado=0 THEN 'Pendiente de aprobación' WHEN estado=1 THEN 'Aprobada' WHEN estado=2 THEN 'Rechazada' ELSE 'No Aplica' END Estado, Acceso,Email, CONVERT(varchar,fecha_registro,105) Fecha,observaciones FROM Op_Ev_Req_E3 Op JOIN Usuario us on op.id_usuario=us.Id_usuario WHERE Op.Id_instalacion=" + CType(Me.Master, Site2).IdInstalacion.ToString() + " AND op.Id_requisito=" + id.ToString() + " AND Op.Activado IS NULL ORDER BY Op.id_Evidencia DESC"
    ...
    Protected Sub gridEvidencia2_RowCommand(sender As Object, e As GridViewCommandEventArgs)
        If e.CommandName = "Aprobar" Then
            ...
            If objUs.Rol = "Constructor/Operador" Or objUs.Rol = "Administrador" Then
                ...
                Dim Query As String = "UPDATE Op_Ev_Req_E3 SET Estado=1 WHERE id_evidencia=" + IdEvidencia.ToString() + ""
                If obj.Modificar(Query) Then
                    'Dim dt = DateTime.ParseExact(DateTime.Now.AddHours(-5).ToLongDateString, "dddd, MMMM dd, yyyy", New CultureInfo("en-US"))

                    'Dim Fecha As String = dt.ToString("D", New CultureInfo("es-ES"))

                    Dim mensaje As String = "La evidencia para el requisito <b>" + Requisito + "</b> fue <b>Aprobada</b>, Instalación: <b>" + CType(Me.Master, Site2).Instalacion.ToString() + "</b>, Etapa: <b>Operación ES</b><hr>" &
        ...
        ElseIf e.CommandName = "Rechazar" Then
            ...
            If objUs.Rol = "Constructor/Operador" Or objUs.Rol = "Administrador" Then
                ...
                Dim Query As String = "UPDATE Op_Ev_Req_E3 SET Estado=2 WHERE id_evidencia=" + IdEvidencia.ToString() + ""
                If obj.Modificar(Query) Then
                    'Dim dt = DateTime.ParseExact(DateTime.Now.AddHours(-5).ToLongDateString, "dddd, MMMM dd, yyyy", New CultureInfo("en-US"))

                    'Dim Fecha As String = dt.ToString("D", New CultureInfo("es-ES"))
                    Dim mensaje As String = "La evidencia para el requisito <b>" + Requisito + "</b> fue <b>Rechazada</b>, Instalación: <b>" + CType(Me.Master, Site2).Instalacion.ToString() + "</b>, Etapa: <b>Operación ES</b><hr>" &
        ...
        ElseIf e.CommandName = "Eliminar" Then
            ...
            If objUs.Rol = "Constructor/Operador" Or objUs.Rol = "Administrador" Then
                ...
                Dim Query As String = "UPDATE Op_Ev_Req_E3 SET ACTIVADO=1 WHERE Id_evidencia=" + IdEvidencia.ToString() + ""
                If obj.Modificar(Query) Then
